...
#
# 检查项:
#   1 frontmatter 完整性（name / description / version / agent_created）
#     目录名比对：容忍外部安装的「name-版本」约定（feishu-doc-1.2.7）；
#     截断/别名目录（SEO (...) -> seo-1.0.3）只给 ℹ，其余不一致仍报 ✗
...
# 安装名：frontmatter 名可做目录 slug 时用它（小写、空格转 -），
# 否则用去掉尾部 -版本号 的规范源目录名（与 sync-skill.sh 同规则）
install_name_of() {
  local f="$1" dirname="$2" fm norm base
  fm=$(fm_field "$f" name)
  norm=$(printf '%s' "$fm" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
  case "$norm" in
    ''|*[!a-z0-9.-]*) : ;;
    *) printf '%s' "$norm"; return 0 ;;
  esac
  base=$(printf '%s' "$dirname" | sed -E 's/-[0-9]+(\.[0-9]+)*$//')
  printf '%s' "$base"
}
...
[ -f "$HOME/.agents/scripts/sync-skill.sh" ] || echo "⚠ 未找到 $HOME/.agents/scripts/sync-skill.sh，下列分发提示暂不可执行"
echo "================================================================"
...
for name in "${TARGETS[@]}"; do
  ...
  if [ -z "${fm_name:-}" ]; then
    ...
    norm=$(printf '%s' "$fm_name" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
    state=bad
    case "$name" in
      "$norm"-[0-9]*) state=ok ;;
      *)
    ...
    case "$state" in
      ...
      alias) issues="$issues\n    ℹ 目录名($name) 为 name($fm_name) 的截断/别名打包" ;;
    esac
  fi
  ...
  # 分发状态（按目录名或安装名匹配，安装名规则见 install_name_of）
  iname=$(install_name_of "$f" "$name")
  marks=""
  for base in "${COPY_TARGETS[@]}" "${LINK_TARGETS[@]}"; do
    if [ -e "$base/$name" ] || { [ "$iname" != "$name" ] && [ -e "$base/$iname" ]; }; then
      marks="$marks $(basename "$(dirname "$base")")"
    fi
  done
